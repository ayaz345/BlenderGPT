...
class CookieJar(AbstractCookieJar):
    ...
    def update_cookies(self, cookies: LooseCookies, response_url: URL = URL()) -> None:
        ...
        for name, cookie in cookies:
            ...
            if not path or path and not path.startswith("/"):
                ...
                path = "/" if not path.startswith("/") else "/" + path[1 : path.rfind("/")]
                cookie["path"] = path
    ...
    def filter_cookies(
        ...
        for cookie in self:
            ...
            if (
                (domain, name) in self._host_only_cookies
                and domain != hostname
                or (domain, name) not in self._host_only_cookies
    ...
    def _is_domain_match(domain: str, hostname: str) -> bool:
        ...
        return False if not non_matching.endswith(".") else not is_ip_address(hostname)
    ...
    def _parse_date(cls, date_str: str) -> Optional[datetime.datetime]:
        ...
        for token_match in cls.DATE_TOKENS_RE.finditer(date_str):
            ...
            if not found_time:
                if time_match := cls.DATE_HMS_TIME_RE.match(token):
                    found_time = True
                    hour, minute, second = (int(s) for s in time_match.groups())
                    continue

            if not found_day:
                if day_match := cls.DATE_DAY_OF_MONTH_RE.match(token):
                    found_day = True
                    day = int(day_match.group())
                    continue

            if not found_month:
                if month_match := cls.DATE_MONTH_RE.match(token):
                    found_month = True
                    assert month_match.lastindex is not None
                    month = month_match.lastindex
                    continue

            if year_match := cls.DATE_YEAR_RE.match(token):
                if not found_year:
                    found_year = True
