...
class _Base:
    ...
    def getall(self, key, default=_marker):
        ...
        if res := [v for i, k, v in self._impl._items if i == identity]:
            return res
    ...
    def __contains__(self, key):
        identity = self._title(key)
        return any(i == identity for i, k, v in self._impl._items)
...
class CIMultiDictProxy(MultiDictProxy):
    ...
    def __init__(self, arg):
        if not isinstance(arg, (CIMultiDict, CIMultiDictProxy)):
            raise TypeError(
                f"ctor requires CIMultiDict or CIMultiDictProxy instance, not {type(arg)}"
            )
...
class MultiDict(_Base, MutableMultiMapping):
    ...
    def _extend(self, args, kwargs, name, method):
        if len(args) > 1:
            raise TypeError(
                f"{name} takes at most 1 positional argument ({len(args)} given)"
            )
        if args:
            arg = args[0]
            if isinstance(arg, (MultiDict, MultiDictProxy)) and not kwargs:
                ...
                for item in arg:
                    if len(item) != 2:
                        raise TypeError(f"{name} takes either dict or list of (key, value) tuples")
...
class _ItemsView(_ViewBase, abc.ItemsView):
    def __contains__(self, item):
        ...
        assert len(item) == 2
        return any(item[0] == k and item[1] == v for i, k, v in self._impl._items)
    ...
    def __repr__(self):
        lst = ["{!r}: {!r}".format(item[1], item[2]) for item in self._impl._items]
        body = ", ".join(lst)
        return f"{self.__class__.__name__}({body})"
...
class _ValuesView(_ViewBase, abc.ValuesView):
    def __contains__(self, value):
        return any(item[2] == value for item in self._impl._items)
    ...
    def __repr__(self):
        lst = ["{!r}".format(item[2]) for item in self._impl._items]
        body = ", ".join(lst)
        return f"{self.__class__.__name__}({body})"
...
class _KeysView(_ViewBase, abc.KeysView):
    def __contains__(self, key):
        return any(item[1] == key for item in self._impl._items)
    ...
    def __repr__(self):
        lst = ["{!r}".format(item[1]) for item in self._impl._items]
        body = ", ".join(lst)
        return f"{self.__class__.__name__}({body})"
